...
class VypujckyFrame:
    ...
    def create_widgets(self):
        ...
        self.pomucka_cb.pack(pady=5)
        self.student_cb.pack(pady=5)

        ttk.Button(self.frame, text="Obnovit seznam", command=self.refresh_combobox).pack(pady=5)
        ttk.Button(self.frame, text="Vytvořit výpůjčku", command=self.create).pack(pady=5)
        ttk.Button(self.frame, text="Vrátit pomůcku", command=self.return_item).pack(pady=5)

        self.tree = ttk.Treeview(
            self.frame,
            columns=("ID", "Pomucka", "Student", "Datum"),
            show="headings"
        )
    ...
    def refresh_combobox(self):
        ...
        self.pomucka_map = {
            f"{row[1]} (ID:{row[0]})": row[0] for row in pomucky
        }
        ...
        self.student_map = {
            f"{row[1]} (ID:{row[0]})": row[0] for row in studenti
        }
    ...
    def refresh(self):
        ...
        cursor.execute("""
            SELECT vypujcky.id,
                   pomucky.nazev,
                   studenti.jmeno || ' ' || studenti.prijmeni,
                   datum_vypujcky
            FROM vypujcky
    ...
    def create(self):
        ...
        cursor.execute("""
            ...
            WHERE pomucka_id=? AND datum_vraceni IS NULL
        """, (pomucka_id,))
        ...
        cursor.execute("""
            INSERT INTO vypujcky (pomucka_id, student_id, datum_vypujcky)
            VALUES (?, ?, ?)
        """, (
            pomucka_id,
            student_id,
